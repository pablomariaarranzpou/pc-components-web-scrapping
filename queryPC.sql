SET @gpu_base = (SELECT perfomance FROM gpu WHERE serie LIKE '%GeForce RTX 3060' ORDER BY perfomance LIMIT 1);
SET @cpu_base = (SELECT perfomances FROM cpu WHERE name LIKE '%Intel%' and name LIKE '%i5%' ORDER BY perfomances LIMIT 1);
SET @ram = 16;
-- SSD en GB
SET @ssd = 200;
-- HDD en TB
...
C.depth AS depth, C.price AS Price FROM bd23.case AS C
JOIN(SELECT DISTINCT width, depth, height, MIN(price) AS price FROM bd23.case
...
POWER.depth = q1.depth AND GPU.price + POWER.price = q1.Price) q2
...
FROM cpu CPU
JOIN cooler COOLER ON COOLER.sockets LIKE CONCAT('%',CPU.socket,'%')
WHERE CPU.perfomances >= @cpu_base
GROUP BY CPU.socket,COOLER.height) q1
WHERE CPU.socket = q1.socket AND COOLER.height = q1.height AND CPU.price + COOLER.price = q1.price) q1
